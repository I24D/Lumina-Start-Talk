...
def search_memory(query: str, limit: int = 8) -> str:
    ...
    memory = load_memory()
    # Tool calls often reuse an indexed key such as ``favorite_color``. Treat
    # underscores as word separators just like _pretty() does for stored keys,
    # otherwise an exact key generated by the model cannot match itself.
    words = [
        word
        for word in re.split(r"[_\W]+", (query or "").lower())
        if len(word) > 1
    ]
